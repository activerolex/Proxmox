...
if whiptail --backtitle "Proxmox VE" --title "$NAME" --yesno "This will create a New $NAME. Proceed?" 10 58; then
  :
else
...
function advanced_settings() {
  ...
  if VM_NAME=$(whiptail --backtitle "Proxmox VE" --inputbox "Set Hostname" 8 58 $HN --title "HOSTNAME" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $VM_NAME ]; then
      HN="$HN"
      echo -e "${DGN}Using Hostname: ${BGN}$HN${CL}"
    else
  ...
  if (whiptail --backtitle "Proxmox VE" --title "ADVANCED SETTINGS COMPLETE" --yesno "Ready to create a $NAME?" --no-button Do-Over 10 58); then
